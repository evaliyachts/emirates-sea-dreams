...
import { mediaRightsRegistry } from "./media-rights";
...
export const approvedServiceRecordSchema = z.object({
  ...
  if (service.path !== `/services/${service.slug}`) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["path"], message: "Service path must be derived from the exact slug." });
  }
  if (new Set(service.yachtIds).size !== 3) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["yachtIds"], message: "Service yacht selections must contain exactly three unique records." });
  }
  if (new Set(service.relatedServiceIds).size !== 3 || service.relatedServiceIds.includes(service.id)) {
    context.addIssue({ code: z.ZodIssueCode.custom, path: ["relatedServiceIds"], message: "Related services must contain three unique other approved owners." });
  }
  const serialized = JSON.stringify(service);
  if (/evaliyachts?|evali yacht|supabase\.co|schema_json_ld|AggregateRating|ratingValue|reviewCount/i.test(serialized)) {
...
const serviceMedia = (rightsRecordId: string, alt: string): ApprovedServiceMedia => {
  ...
  return approvedServiceMediaSchema.parse({
    path: source.path,
    alt,
    width: source.width,
...
const approved = (record: ApprovedServiceRecord) => approvedServiceRecordSchema.parse(record);
